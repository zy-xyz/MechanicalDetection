...
cls_name = ["轴承", "螺栓", "法兰", "齿轮", "螺母", "弹簧"]  # 定义类名列表

model = YOLOv8v5Detector()  # 创建YOLOv8Detector对象
model.load_model(abs_path("weights/mechanical-yolov8n.pt", path_type="current"))  # 加载预训练的YOLOv8模型
colors = get_cls_color(model.names)  # 获取类别颜色

app = QtWidgets.QApplication(sys.argv)  # 创建QApplication对象
window = MainWindow()  # 创建MainWindow对象

filename = abs_path("test_media/3-机械元件.mp4", path_type="current")  # 定义视频文件的路径
