...
class RecMainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent=None, *args, **kwargs):
        ...
        QWindowCtrls(self, exit_title="基于YOLOv8/v5的机械器件识别系统",  # 退出确认框标题
        ...
        self.count_name = ["轴承", "螺栓", "法兰", "齿轮", "螺母", "弹簧"]  # 定义类名列表
        ...
        # 加载训练的模型权重
        self.model.load_model(model_path=abs_path("weights/mechanical-yolov8n.pt", path_type="current"))
        self.plot_horizontal_bar(self.label_bar, self.count_name, [0 for i in self.count_name], self.colors, margin=20)
            ...
            if col == 0:  # 只在点击第一列时执行操作
                ...
                if (this_path is not None) & (res is not None) & (axes is not None):
                    ...
                    if os.path.isfile(this_path):  # 检查文件路径是否存在
                        ...
                        image = cv2.resize(image, (850, 500))  # 调整图片大小
                        ...
                        self.plot_vertical_bar(self.label_bar, self.count_name, count, self.colors, margin=20)
    ...
    def frame_process(self, image):
        ...
        # pre_img = image.copy()
        image = cv2.resize(image, (850, 500))  # 调整图像大小
        ...
        if det is not None and len(det):
            ...
            if len(det_info):
                ...
                self.plot_horizontal_bar(self.label_bar, self.count_name, count, self.colors,
                                         margin=20)
